...
<div class="eob-page">
    <div class="eob-topbar">
        <div>
            <div class="eob-title">Edit Candidate Profile</div>
            <div class="eob-breadcrumb">
                <a href="{{ route('recruitment.index') }}">Recruitment</a> > 
                <a href="{{ route('recruitment.show', $candidate) }}">{{ $candidate->candidate_no }}</a> > Edit
            </div>
        </div>
        <div class="eob-actions">
            <a href="{{ route('recruitment.show', $candidate) }}" class="eob-btn eob-btn-ghost">Cancel</a>
        </div>
    </div>
    ...
    <div class="eob-body">
        ...
        <form method="POST" action="{{ route('recruitment.update', $candidate) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="rec-section-card">
                <div class="rec-section-head">
                    ...
                    <div class="rec-chip rec-chip-applied">{{ $candidate->candidate_no }}</div>
                </div>
                <div class="eob-card-body">
                    <div class="eob-form-grid">
                        <div class="eob-group">
                            ...
                            <input type="text" name="name" class="eob-input @error('name') is-invalid @enderror" value="{{ old('name', $candidate->name) }}" placeholder="Full Candidate Name" required>
                        ...
                        <div class="eob-group">
                            ...
                            <input type="text" name="mobile_number" class="eob-input @error('mobile_number') is-invalid @enderror" value="{{ old('mobile_number', $candidate->mobile_number) }}" placeholder="10 digit mobile number" required>
                            @error('mobile_number')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Email Address</label>
                            <input type="email" name="email" class="eob-input @error('email') is-invalid @enderror" value="{{ old('email', $candidate->email) }}" placeholder="candidate@example.com">
                        ...
                        <div class="eob-group">
                            ...
                            <input type="text" name="job_title" class="eob-input @error('job_title') is-invalid @enderror" value="{{ old('job_title', $candidate->job_title) }}" placeholder="PHP Developer, HR Executive..." required>
                            @error('job_title')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Expected CTC (₹)</label>
                            <input type="number" step="0.01" min="0" name="expected_ctc" class="eob-input @error('expected_ctc') is-invalid @enderror" value="{{ old('expected_ctc', $candidate->expected_ctc) }}" placeholder="e.g. 350000">
                        ...
                        <div class="eob-group">
                            ...
                            <select name="source" id="sourceSelect" class="eob-select @error('source') is-invalid @enderror" onchange="toggleSourceDetails()">
                                ...
                                <option value="Mail" @selected(old('source', $candidate->source) === 'Mail')>Mail</option>
                                <option value="Whatsapp" @selected(old('source', $candidate->source) === 'Whatsapp')>Whatsapp</option>
                                <option value="Naukri" @selected(old('source', $candidate->source) === 'Naukri')>Naukri</option>
                                <option value="Others" @selected(old('source', $candidate->source) === 'Others')>Others</option>
                            </select>
                            @error('source')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>

                        <div class="eob-group" id="sourceDetailsGroup" style="display: {{ old('source', $candidate->source) === 'Others' ? 'flex' : 'none' }};">
                            <label class="eob-label">Other Source Details <span class="eob-label-required">*</span></label>
                            <input type="text" name="source_details" id="sourceDetailsInput" class="eob-input @error('source_details') is-invalid @enderror" value="{{ old('source_details', $candidate->source_details) }}" placeholder="Specify source details...">
            ...
            <div class="rec-section-card">
                ...
                <div class="eob-card-body">
                    <div class="rec-type-selector">
                        <label class="rec-type-label {{ old('candidate_type', $candidate->candidate_type) === 'fresher' ? 'active' : '' }}" id="typeLabelFresher">
                            <input type="radio" name="candidate_type" value="fresher" onchange="toggleCandidateType()" {{ old('candidate_type', $candidate->candidate_type) === 'fresher' ? 'checked' : '' }}>
                            Fresher
                        </label>
                        <label class="rec-type-label {{ old('candidate_type', $candidate->candidate_type) === 'intern' ? 'active' : '' }}" id="typeLabelIntern">
                            <input type="radio" name="candidate_type" value="intern" onchange="toggleCandidateType()" {{ old('candidate_type', $candidate->candidate_type) === 'intern' ? 'checked' : '' }}>
                            Intern
                        </label>
                        <label class="rec-type-label {{ old('candidate_type', $candidate->candidate_type) === 'experienced' ? 'active' : '' }}" id="typeLabelExperienced">
                            <input type="radio" name="candidate_type" value="experienced" onchange="toggleCandidateType()" {{ old('candidate_type', $candidate->candidate_type) === 'experienced' ? 'checked' : '' }}>
                            Experienced
                        </label>
                    </div>
                </div>
            ...
            <div class="rec-section-card" id="internSectionCard" style="display: {{ old('candidate_type', $candidate->candidate_type) === 'intern' ? 'block' : 'none' }};">
                <div class="rec-section-head">
                ...
                <div class="eob-card-body">
                    <div class="eob-form-grid">
                        <div class="eob-group">
                            ...
                            <input type="text" name="institute_name" class="eob-input @error('institute_name') is-invalid @enderror" value="{{ old('institute_name', $candidate->institute_name) }}" placeholder="e.g. Loyola College, Anna University...">
                            @error('institute_name')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Course / Degree</label>
                            <input type="text" name="course_name" class="eob-input @error('course_name') is-invalid @enderror" value="{{ old('course_name', $candidate->course_name) }}" placeholder="e.g. B.E CSE, B.Sc IT, BCA...">
                            @error('course_name')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Internship Duration (Months)</label>
                            <input type="text" name="internship_months" class="eob-input @error('internship_months') is-invalid @enderror" value="{{ old('internship_months', $candidate->internship_months) }}" placeholder="e.g. 1 Month, 3 Months, 6 Months...">
                        ...
                        <div class="eob-group">
                            ...
                            <select name="has_stipend" id="hasStipendSelect" class="eob-select @error('has_stipend') is-invalid @enderror" onchange="toggleStipendAmount()">
                                ...
                                <option value="yes" @selected(old('has_stipend', $candidate->has_stipend) === 'yes')>Yes</option>
                                <option value="no" @selected(old('has_stipend', $candidate->has_stipend) === 'no')>No</option>
                            </select>
                            @error('has_stipend')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group" id="stipendAmountGroup" style="display: {{ old('has_stipend', $candidate->has_stipend) === 'yes' ? 'flex' : 'none' }};">
                            <label class="eob-label">Stipend Amount (₹)</label>
                            <input type="number" step="0.01" min="0" name="stipend_amount" class="eob-input @error('stipend_amount') is-invalid @enderror" value="{{ old('stipend_amount', $candidate->stipend_amount) }}" placeholder="e.g. 5000">
            ...
            <div class="rec-section-card" id="experienceSectionCard" style="display: {{ old('candidate_type', $candidate->candidate_type) === 'experienced' ? 'block' : 'none' }};">
                <div class="rec-section-head">
                ...
                <div class="eob-card-body">
                    <div class="eob-form-grid">
                        <div class="eob-group">
                            ...
                            <input type="number" name="experience_years" min="0" max="60" class="eob-input @error('experience_years') is-invalid @enderror" value="{{ old('experience_years', $candidate->experience_years) }}" placeholder="e.g. 2">
                            @error('experience_years')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Notice Period</label>
                            <input type="text" name="notice_period" class="eob-input @error('notice_period') is-invalid @enderror" value="{{ old('notice_period', $candidate->notice_period) }}" placeholder="Immediate, 15 Days, 30 Days...">
                            @error('notice_period')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Current CTC (₹)</label>
                            <input type="number" step="0.01" min="0" name="current_ctc" class="eob-input @error('current_ctc') is-invalid @enderror" value="{{ old('current_ctc', $candidate->current_ctc) }}" placeholder="e.g. 250000">
                            @error('current_ctc')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Previous Company Name</label>
                            <input type="text" name="previous_company" class="eob-input @error('previous_company') is-invalid @enderror" value="{{ old('previous_company', $candidate->previous_company) }}" placeholder="e.g. ABC Technologies">
                            @error('previous_company')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Previous Company HR Name</label>
                            <input type="text" name="previous_hr_name" class="eob-input @error('previous_hr_name') is-invalid @enderror" value="{{ old('previous_hr_name', $candidate->previous_hr_name) }}" placeholder="HR Manager Name">
                        ...
                        <div class="eob-group">
                            ...
                            <input type="text" name="previous_hr_contact" class="eob-input @error('previous_hr_contact') is-invalid @enderror" value="{{ old('previous_hr_contact', $candidate->previous_hr_contact) }}" placeholder="HR Phone Number">
                            @error('previous_hr_contact')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group">
                            <label class="eob-label">Relieving Reason</label>
                            <input type="text" name="relieving_reason" class="eob-input @error('relieving_reason') is-invalid @enderror" value="{{ old('relieving_reason', $candidate->relieving_reason) }}" placeholder="Reason for leaving previous job">
                        ...
                        <div class="eob-group">
                            ...
                            <select name="has_laptop" class="eob-select @error('has_laptop') is-invalid @enderror">
                                <option value="">Select Option</option>
                                <option value="yes" @selected(old('has_laptop', $candidate->has_laptop) === 'yes')>Yes</option>
                                <option value="no" @selected(old('has_laptop', $candidate->has_laptop) === 'no')>No</option>
                            </select>
                    ...
                    <div class="eob-table-wrap">
                        <table class="eob-table rec-edu-tbl" id="educationTable">
                            ...
                            <tbody id="educationTableBody">
                                @php
                                    $oldEdu = old('education_details', $candidate->education_details ?? []);
                                    if (empty($oldEdu)) {
                                        $oldEdu = [
                ...
                <div class="eob-card-body">
                    <div class="eob-form-grid">
                        <div class="eob-group full">
                            ...
                            <div class="eob-help">Supported formats: PDF, DOC, DOCX up to 5 MB. Leave empty to keep existing resume.</div>
                            @error('resume')<div class="eob-error">{{ $message }}</div>@enderror
                        </div>
                        <div class="eob-group full">
                            <label class="eob-label">Remarks / Notes</label>
                            <textarea name="remarks" class="eob-textarea @error('remarks') is-invalid @enderror" placeholder="Initial notes from HR, referral details, screening context...">{{ old('remarks', $candidate->remarks) }}</textarea>
            ...
            <div class="eob-foot" style="background:#fff; border-radius:16px; border:1px solid #e1dee3; margin-bottom:30px;">
                <a href="{{ route('recruitment.show', $candidate) }}" class="eob-btn eob-btn-ghost">Cancel</a>
                <button type="submit" class="eob-btn eob-btn-primary">Update Candidate Profile</button>
            </div>
        </form>
    </div>
</div>
