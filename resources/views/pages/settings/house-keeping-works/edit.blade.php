@extends('layouts.app')
@section('title', 'Edit House Keeping Work')
...
<main class="main-content">
    <div class="crm-page-body" style="max-width:760px;">
        <div class="crm-page-header">
            <div>
                <h2 class="crm-title">Edit House Keeping Work</h2>
                <p class="crm-subtitle">Update category selection and work entry text.</p>
            </div>
        ...
        <div class="crm-table-wrap" style="padding:20px;">
            <form method="POST" action="{{ route('settings.house-keeping-works.update', $houseKeepingWork) }}" style="display:grid; gap:16px;">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <select name="house_keeping_category_id" class="crm-input" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" @selected((string) old('house_keeping_category_id', $houseKeepingWork->house_keeping_category_id) === (string) $category->id)>{{ $category->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="work_name" class="crm-input" value="{{ old('work_name', $houseKeepingWork->work_name) }}" required>
                ...
                <div>
                    ...
                    <textarea name="notes" class="crm-input" rows="4">{{ old('notes', $houseKeepingWork->notes) }}</textarea>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" class="crm-input" min="0" value="{{ old('sort_order', $houseKeepingWork->sort_order) }}">
                    @error('sort_order')<div style="margin-top:6px;color:#dc2626;font-size:12px;">{{ $message }}</div>@enderror
                </div>
                <div style="display:flex; gap:10px;">
                    <button type="submit" class="crm-btn crm-btn-primary">Update Work</button>
