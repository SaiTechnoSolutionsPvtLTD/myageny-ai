...
<main class="main-content">
    <div class="crm-page-body">
        <div class="crm-page-header">
            <div>
                <h2 class="crm-title">Edit Branch</h2>
                <p class="crm-subtitle">Update branch details and default branch preference.</p>
            </div>
            ...
            <div class="crm-form-wrap">
                ...
                <div class="crm-form-body">
                    <div>
                        ...
                        <input type="text" name="name" class="crm-input" value="{{ old('name', $branch->name) }}" required>
                    </div>
                    <div>
                        <label class="crm-label">Code</label>
                        <input type="text" name="code" class="crm-input" value="{{ old('code', $branch->code) }}">
                    </div>
                    <div>
                        <label class="crm-label">Branch Type</label>
                        <select name="branch_type" class="crm-input">
                            <option value="">Select Branch Type</option>
                            <option value="COCO" @selected(old('branch_type', $branch->branch_type) === 'COCO')>COCO</option>
                            <option value="NON COCO" @selected(old('branch_type', $branch->branch_type) === 'NON COCO')>NON COCO</option>
                        </select>
                    </div>
                    <div>
                        <label class="crm-label">City</label>
                        <input type="text" name="city" class="crm-input" value="{{ old('city', $branch->city) }}">
                    </div>
                    <div>
                        <label class="crm-label">State</label>
                        <input type="text" name="state" class="crm-input" value="{{ old('state', $branch->state) }}">
                    </div>
                    <div>
                        <label class="crm-label">Phone</label>
                        <input type="text" name="phone" class="crm-input" value="{{ old('phone', $branch->phone) }}">
                    </div>
                    <div>
                    ...
                    <div>
                        <label style="display:flex; align-items:center; gap:10px;">
                            <input type="checkbox" name="is_default" value="1" @checked(old('is_default', $branch->is_default))>
                ...
                <div class="crm-form-foot">
                    ...
                    <button type="submit" class="crm-btn crm-btn-primary">Update Branch</button>
                </div>
            </div>
        </form>
    </div>
