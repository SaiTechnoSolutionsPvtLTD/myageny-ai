...
class ProjectApiController extends Controller
{
    ...
    public function dashboard(Request $request): JsonResponse
    {
        ...
        $employees = User::query()
            ...
            ->map(function ($emp) use ($onboardingDepts, $internDepts) {
                ...
                return [
                    'id' => $emp->id,
                    'name' => $emp->name,
                    'department_ids' => collect(array_merge($roleDeptIds, $eoDeptIds, $ijfDeptIds))
                        ->filter()
                        ->unique()
                        ->values()
                        ->all(),
        ...
        $parsedFrom = $this->parseFilterDate($dateFromStr);
        $parsedTo = $this->parseFilterDate($dateToStr);
        $formattedDateFrom = $parsedFrom ? $parsedFrom->toDateString() : '';
        $formattedDateTo = $parsedTo ? $parsedTo->toDateString() : '';

        $dateRangeLabel = '';
        if ($parsedFrom && $parsedTo) {
            $dateRangeLabel = $parsedFrom->format('d/m/Y') . ' - ' . $parsedTo->format('d/m/Y');
        } elseif ($parsedFrom) {
            $dateRangeLabel = 'From ' . $parsedFrom->format('d/m/Y');
        } elseif ($parsedTo) {
            $dateRangeLabel = 'Up to ' . $parsedTo->format('d/m/Y');
        }
        ...
        $projectStatuses = [
            ...
            ['key' => 'ongoing', 'label' => 'Ongoing / Onboard'],
            ['key' => 'hold', 'label' => 'Hold'],
        ...
        $quickDates = [
            ...
            ['key' => 'custom_onboarding', 'label' => 'Custom Dates (Onboarding)'],
            ['key' => 'custom_delivery', 'label' => 'Custom Dates (Delivery)'],
        ];

        $availableDashboards = $canViewSwitcher ? ['development', 'dm', 'design', 'testing'] : [];
        ...
        return response()->json([
            ...
            'data'    => [
                ...
                'results_count'                => $filteredProjects->count(),
                ...
                'delivery_section_title'       => $deliverySectionTitle,
                'delivery_section_badge'       => $deliverySectionBadge,
    ...
    {
        $df = $this->parseFilterDate($filters['date_from'] ?? '');
        $dt = $this->parseFilterDate($filters['date_to'] ?? '');

        if ($df && $dt) {
            $start = $df->copy()->startOfDay();
